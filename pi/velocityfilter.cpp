...
Particle::Evolve(double Timestep, double GyroVelocity, double GyroWidth, double ForceWidth)
{
   ...
   double NewVelocity = GyroWidth*randutil::randn() + (GyroVelocity-GyroOffset);

   // now determine what the total force should be to achieve this velocity.
   // We could use a 2nd order or 3rd order approximation.  For a start, use 2nd order.
   // This assumes the force is piece-wise constant

   double NewForce = (NewVelocity - Velocity) / Timestep;

   // new dynamical state
   double NewTheta = Theta + 0.5*(NewVelocity+Velocity)*Timestep + 0.5*NewForce*Timestep*Timestep;

   //   std::cerr << Timestep << ' ' << to_deg(GyroVelocity) << ' ' << to_deg(Velocity) << ' ' << to_deg(NewVelocity) << ' ' << to_deg(Theta) << ' ' << to_deg(NewTheta) << '\n';

   // split out the firce into different components
   double th = 0.5*(Theta+NewTheta);
   ForceDynamical = -(g/l_b) * std::sin(th) - 0.5*(Velocity+NewVelocity)*k_b;
   ForceRope = 0;
   ...
   if (th > hstay)
   {
      ForceStay = -((th - hstay)*Y + Velocity*k_s);
   }
   else if (th < bstay)
   {
      ForceStay = -((th - bstay)*Y + Velocity*k_s);
   }
   else
   {
      ForceStay = 0;
   ...
   double NewForceExternal = Force - ForceDynamical - ForceRope - ForceStay;
   ForceDot = (NewForceExternal-ForceExternal) / Timestep;
   // Update the weight based on the external forces
   //   Weight *= NormalPDF(NewForceExternal, ForceWidth, 0);
   // also want to weight by ForceDot
   Weight *= NormalPDF(ForceDot, 10000, 0);
   // distribution for the impulse
   //Weight *= NormalPDF(NewForce*NewVelocity, 400, 0);

   // force on the stay shouldn't exceed breaking point!  What is the breaking angle of the stay?
   Weight *= NormalPDF(ForceStay, Y*1*pi/180, 0);

   // update dynamical state
   Theta = NewTheta;
   Velocity = NewVelocity;
   Force = NewForce;
   ForceExternal = NewForceExternal;
}
